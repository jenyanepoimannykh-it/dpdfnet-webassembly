...
def main() -> None:
    ...
    denoised = dereverb(degraded.astype(np.float32), meta, session)
    ...
    out.write_text(
        f"""// The idle chart is a real measurement, not an illustration, and it is measured from the
// very clip the page offers to play. {args.seconds:.0f} seconds of speech under pink noise at {args.noise_snr:.0f} dB SNR
// in a room (RT60 {args.rt60} s, direct-to-reverberant {args.drr:+.0f} dB), run through this model by
